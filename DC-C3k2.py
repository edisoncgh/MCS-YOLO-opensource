...
class DualConvModule(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, g=4):
        super(DualConvModule, self).__init__()
        # Group Convolution
...
class DC_BottleNeck(nn.Module):
    # ch_in, ch_out, shortcut, groups, kernels, expand
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        super().__init__()
        c_ = int(c2 * e)
        self.cv1 = Conv(c1, c_, k[0], 1)
        self.cv2 = DualConvModule(c_, c2)
        self.add = shortcut and c1 == c2
...
class DC_C3k(C3):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        ...
        c_ = int(c2 * e)  
        self.m = nn.Sequential(*(DC_BottleNeck(c_, c_, shortcut, g, k=(k, k), e=1.0) for _ in range(n)))

class DC_C3k2(C2f):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            DC_C3k(self.c, self.c, 2, shortcut, g) if c3k else DC_BottleNeck(self.c, self.c, shortcut, g) for _ in range(n)
        )
